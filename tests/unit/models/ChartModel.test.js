...
describe('PROCESS_LABELS', () => {
  it('should contain labels for all 8 processes', () => {
    ...
    expect(PROCESS_LABELS).toHaveProperty('s_fit');
    expect(PROCESS_LABELS).toHaveProperty('ass_bal');
...
describe('PROCESS_KEY_MAP', () => {
  ...
  it('should map uppercase snake_case to lowercase snake_case', () => {
    // PROCESS_KEY_MAP maps uppercase to lowercase with underscores preserved
    expect(PROCESS_KEY_MAP['S_CUT']).toBe('s_cut');
    expect(PROCESS_KEY_MAP['PRE_SEW']).toBe('pre_sew');
    expect(PROCESS_KEY_MAP['SEW_INPUT']).toBe('sew_input');
    expect(PROCESS_KEY_MAP['SEW_BAL']).toBe('sew_bal');
    expect(PROCESS_KEY_MAP['WH_IN']).toBe('wh_in');
    expect(PROCESS_KEY_MAP['WH_OUT']).toBe('wh_out');
  });
...
describe('analyzeDailyReport', () => {
  ...
  it('should handle empty data array', () => {
    ...
    // processStats may have bottlenecks when no data (0% rate for all)
    expect(result.processStats.length).toBeLessThanOrEqual(8);
    // bottlenecks may be up to 3 when all processes have 0% rate
    expect(result.bottlenecks.length).toBeLessThanOrEqual(3);
...
describe('analyzeMonthlyReport', () => {
  ...
  it('should order monthlyTrends chronologically (oldest to newest)', () => {
    ...
    // Should be chronological, containing 6 months
    expect(result.monthlyTrends.length).toBe(6);
    // Each month should be in YYYY-MM format
    result.monthlyTrends.forEach(trend => {
      expect(trend.month).toMatch(/^\d{4}-\d{2}$/);
    });
    // Trends should be chronologically ordered (ascending)
    for (let i = 1; i < result.monthlyTrends.length; i++) {
    ...
    const data = [
      {
        ...
        production: {
          ...
          sew_input: { status: 'completed' },
          sew_bal: { status: 'completed' },
          s_fit: { status: 'completed' },
          ass_bal: { status: 'completed' },
          wh_in: { status: 'completed' },
          wh_out: { status: 'completed' },
        },
      },
  ...
  it('should use PROCESS_KEY_MAP for processStats keys', () => {
    ...
    // ProcessStats should have 8 entries
    expect(result.processStats.length).toBe(8);
    // Check that process names are Korean
    const processNames = result.processStats.map(p => p.process);
    expect(processNames.some(name => name.includes('재단') || name.includes('S_CUT'))).toBe(true);
  });
