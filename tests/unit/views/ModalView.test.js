...
describe('ModalView Module', () => {
  ...
  describe('showOrderListModal', () => {
    it('should show order list modal', () => {
      expect(() => {
        showOrderListModal('Test Orders', mockOrderData);
      }).not.toThrow();
    });

    it('should filter orders when needed', () => {
      // Note: showOrderListModal takes (title, orders) - filtering should be done before calling
      const filteredOrders = mockOrderData.filter(d => d.factory === 'A');

      expect(() => {
        showOrderListModal('Factory A Orders', filteredOrders);
      }).not.toThrow();

      // Verify filtered data was used (fewer orders)
      expect(filteredOrders.length).toBeLessThanOrEqual(mockOrderData.length);
    });

    it('should handle empty orders array', () => {
      expect(() => {
        showOrderListModal('Empty Orders', []);
      }).not.toThrow();
    });
  });
  ...
  describe('closeOrderModal', () => {
    it('should close order modal', () => {
      showOrderListModal('Test', mockOrderData);
...
describe('ModalView Edge Cases', () => {
  ...
  it('should handle XSS attempts in modal content', () => {
    expect(() => {
      showInfoModal('<script>alert("title")</script>', '<img src=x onerror=alert("content")>');
    }).not.toThrow();
  });
